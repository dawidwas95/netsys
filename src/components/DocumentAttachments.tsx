...
import { Upload, FileText, Image, Trash2, Download, Eye, Loader2, Paperclip } from "lucide-react";
...
export const DocumentAttachments = forwardRef<DocumentAttachmentsHandle, DocumentAttachmentsProps>(
  function DocumentAttachments({ documentId, compact = false }, ref) {
    const { user } = useAuth();
    ...
    function handleFilesSelected(fileList: FileList | null) {
      ...
      if (documentId) {
        ...
        // Buffer files for upload after document creation
        setPendingFiles(prev => [...prev, ...files]);
        toast.success(`${files.length} plik(ów) dodano do kolejki — zostaną przesłane po zapisaniu dokumentu`);
      }
    }
    ...
    return (
      <div className="space-y-3">
        ...
        <input
          ...
          type="file"
          multiple
          accept=".pdf,.jpg,.jpeg,.png,.heic,.gif,.webp,.doc,.docx,.xls,.xlsx,.txt,.tiff,.bmp"
          className="hidden"
        ...
        >
          {uploading ? (
            ...
            <div className="flex flex-col items-center gap-2 py-1">
              ...
              <p className="text-[11px] text-muted-foreground">PDF, JPG, PNG, HEIC, skany dokumentów</p>
              {!documentId && (
                <p className="text-[11px] text-primary font-medium">Pliki zostaną przesłane po zapisaniu dokumentu</p>
              )}
            </div>
          )}
        </div>

        {/* Pending files (buffered for new documents) */}
        {pendingFiles.length > 0 && !documentId && (
          <div className="space-y-1.5">
            <p className="text-xs font-medium text-muted-foreground">Pliki do przesłania ({pendingFiles.length}):</p>
            {pendingFiles.map((file, idx) => (
              <div key={idx} className="flex items-center gap-3 rounded-lg border border-dashed border-primary/40 bg-primary/5 p-2.5 text-sm">
                {getFileIcon(file.type)}
                <div className="flex-1 min-w-0">
                  <p className="font-medium truncate text-sm">{file.name}</p>
                  <p className="text-xs text-muted-foreground">{formatFileSize(file.size)} · oczekuje na zapis</p>
                </div>
                <Button
                  type="button"
                  variant="ghost"
...
/** Upload buffered files to a newly created document */
